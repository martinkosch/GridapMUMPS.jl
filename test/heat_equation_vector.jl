# Example adapted from https://github.com/gridap/Gridap.jl/blob/master/test/ODEsTests/TransientFEProblemsTests/HeatEquationVectorTests.jl for Gridap 18.02
...
tol = 1.0e-6
lusolver = LUSolver()
sysslvr_nl_lu = NLSolver(lusolver, show_trace = false, method = :newton, iterations = 10)
mumps = MUMPSSolver(mumps_unsymmetric, default_icntl, default_cntl64)
sysslvr_nl_mumps = NLSolver(mumps, show_trace = false, method = :newton, iterations = 10)
odeslvrs = (
    ThetaMethod(sysslvr_nl_mumps, dt, 0.5),
    ThetaMethod(sysslvr_nl_lu, dt, 0.5)
)
